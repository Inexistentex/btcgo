...
func GeneratePublicKey(privKeyBytes []byte) []byte {
    ...
    if len(privKeyBytes) != 32 {
        panic("Chave privada inválida. Deve ter 32 bytes.")
    }

    privKey := secp256k1.PrivKeyFromBytes(privKeyBytes) // Obter a chave privada

    pubKey := privKey.PubKey()                            // Gerar a chave pública
    return pubKey.SerializeCompressed()                   // Retornar a chave pública comprimida
}
