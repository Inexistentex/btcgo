...
func main() {
	...
	wallet := ranges.Ranges[rangeNumber-1].Status
	...
	// Calcular a quantidade de possíveis chaves no intervalo
	numPossibleKeys := new(big.Int).Sub(maxPrivKeyInt, privKeyInt)
	numPossibleKeys.Add(numPossibleKeys, big.NewInt(1)) // Incluir o limite superior no intervalo
	fmt.Printf("Quantidade de possíveis chaves no intervalo: %s\n", numPossibleKeys.String())
	fmt.Printf("Wallet a ser buscada: %s\n", wallet)
