...
func getRandomBlockWithVariableSize(minPrivKey, maxPrivKey *big.Int, initialBlockSize int64, intervalTree *IntervalTree) (*big.Int, int64) {
    ...
    for blockSize > 0 {
        ...
        // Verificar caracteres repetidos no início e final do intervalo
        if hasRepeatedChars(block.Text(16), 3) || hasRepeatedChars(blockEnd.Text(16), 3) {
            continue
        }
...
func searchInBlock(wallets []string, block *big.Int, actualBlockSize int64, stopSignal chan struct{}, keysChecked *int64, startTime time.Time) {
	for i := int64(0); i < actualBlockSize; i++ {
		...
		default:
			...
			if hasRepeatedChars(privKeyHex, 3) {
				continue
			}
			...
			*keysChecked++
			if *keysChecked%actualBlockSize == 0 {
				elapsedTime := time.Since(startTime).Seconds()
				rate := float64(*keysChecked) / elapsedTime
