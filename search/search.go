...
var (
	assignedBlocks   sync.Map // Usar sync.Map para armazenamento de blocos atribuídos
	blockCounter     int64
...
func SearchInBlocks(wallets []string, blocksRead *int64, blockSize int64, minPrivKey, maxPrivKey *big.Int, stopSignal chan struct{}, startTime time.Time, intervalTree *IntervalTree, keysChecked *int64, id int) {
	for {
		...
		default:
			...
			// Atualizar contador de blocos de forma atômica
			blockNumber := atomic.AddInt64(&blockCounter, 1)
			assignedBlocks.Store(id, Interval{Min: block, Max: new(big.Int).Add(block, big.NewInt(actualBlockSize-1))})

			searchInBlock(wallets, block, actualBlockSize, stopSignal, keysChecked, startTime)

			assignedBlocks.Delete(id)

			saveProgress(*blocksRead, block, new(big.Int).Add(block, big.NewInt(actualBlockSize-1)), int(blockNumber))
...
func searchInBlock(wallets []string, block *big.Int, actualBlockSize int64, stopSignal chan struct{}, keysChecked *int64, startTime time.Time) {
	for i := int64(0); i < actualBlockSize; i++ {
		...
		default:
			...
			privKeyBytes := privKey.FillBytes(make([]byte, 32)) // Garantir que a chave tenha 32 bytes
			pubKey := wif.GeneratePublicKey(privKeyBytes)
			address := wif.PublicKeyToAddress(pubKey)

			if contains(wallets, address) {
